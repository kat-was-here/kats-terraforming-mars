...
export class MindSetMars extends CorporationCard implements ICorporationCard {
  constructor() {
    super({
      ...
      metadata: {
        cardNumber: 'PfC21',
        description: 'You start with 44 M€ and 1 agenda resource on this card.',
        renderData: CardRenderer.builder((b) => {
          ...
          b.text('(Action: Spend 5 agenda resources to place a city tile on Mars.)', Size.SMALL, false, false).br;
          b.text('(Action: Spend 8 M€ (steel may be used) to draw a building card.)', Size.SMALL, false, false);
        }),
      },
    });
  }
  ...
  private canDrawBuildingCard(player: IPlayer) {
    return player.canAfford({cost: 8, steel: true});
  }

  public canAct(player: IPlayer) {
    return this.canAddDelegate(player) || this.canAddCity(player) || this.canDrawBuildingCard(player);
  }
  ...
  public action(player: IPlayer) {
    ...
    if (this.canDrawBuildingCard(player)) {
      options.options.push(new SelectOption('Spend 8 M€ (steel may be used) to draw a building card').andThen(() => {
    ...
    if (options.options.length === 1) return options.options[0];
    return options;
  }
}
